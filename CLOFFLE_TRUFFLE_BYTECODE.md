...
**Suggested next steps (2026-03):** (1) **`CoreBytecodeLoadSmokeTest`** now gets past **`(defmacro when …)`** (recur arg evaluation order + `CreateClosure` arity + `GetRestArgs` `RestArgs` unwrap fixed); next failure is **`FrameSlotTypeException`** at **`core.clj:827`** (`=` definition) — likely a primitive / unboxed local issue that the bytecode path's object-only frame slots don't handle. (2) **Roadmap #5** — **`NewInstanceExpr`** / **`reify`** / **`deftype`** / **`proxy`**. (3) **Integration** — grow **`bootstrap_slice.clj`**, **`run-bytecode-require-ns-integration`**, and optional **AOT deserialize** per **Full Integration** below.
