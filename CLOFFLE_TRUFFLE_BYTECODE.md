...
| Order | Target | Notes |
| ----- | ------ | ----- |
| **1** | **`loop*` / `recur`** (`RecurExpr`, `LetExpr.isLoop`) | Real **`core.clj`** uses **`recur` very early**—e.g. the variadic branch of **`conj`** uses **`recur`**. **`core_mini.clj`** avoids that by simplifying **`conj`**. The Truffle Bytecode DSL still **does not reliably support arbitrary backward branches**; see **RecurExpr** under **Implemented Expressions** and **Pending → ExprToBytecode**. |
...
| **3** | **`locking` → monitors** (`MonitorEnterExpr` / `MonitorExitExpr`) | **`(locking x …)`** lowers to JVM monitor enter/exit; **`core.clj`** defines **`locking`** and uses it e.g. around **`RT/REQUIRE_LOCK`** during **`require`**. Ordering of enter/exit must match the JVM. |
| **4** | **`letfn*`** (`LetFnExpr`) | The **`letfn`** macro expands to **`letfn*`**; used for local mutual recursion in later parts of **`core.clj`**. |
...
| **—** | **Runtime integration** | **`ns` / `require`**, and choosing **deserialize bytecode vs `ExprToNode`** per the **RT** policy and **Full Integration** section—not only more `Compiler.Expr` mappings. |

The granular **`Expr`** gaps (**`LetFnExpr`**, monitors, **`UnresolvedVarExpr`**, fragile **`RecurExpr`**) stay listed under **Pending / To Do** below.
...
- **Build**: `clojure -T:build run-bytecode-dsl-tests` runs both JUnit classes by default (optional `:fresh`, `:args` for JUnit discovery). Reports under `target/surefire-reports`. The suite grows with new `Expr` mappings; run it after bytecode or `Compiler` changes.
...
Run `clojure -T:build run-bytecode-dsl-tests` (default selects `ExprToBytecodeTest` and `ExprToBytecodeSourceLocationTest`). These pass today; they are the practical “no `clojure.core`” surface for analyzer + bytecode (not an exhaustive list of every `Compiler.Expr` type). **Coverage includes** (among others): literals and collections, `if`/`do`/`quote`, `let*`/`fn*` (multi-arity, rest), `def`/`var`, `try`/`catch`/`finally`/`throw`, Java `new` / interop / `instance?`, `KeywordInvokeExpr`, `AssignExpr` on Java fields, `MetaExpr`, `clojure.core/import*` (two-phase eval for short `new` names), `QualifiedMethodExpr` (`Long/valueOf` as value + invoke), `case*` / `CaseExpr` (see table row). Source attachment and bytecode serialization of `Source` are covered in **`ExprToBytecodeSourceLocationTest`**.
...
**Implemented and covered in `ExprToBytecodeTest`** (non-exhaustive): `CaseExpr` (`case*`), `ImportExpr`, `QualifiedMethodExpr` (via `buildThunkFnStar`), `KeywordInvokeExpr`, `AssignExpr` (vars / fields / locals), multi-arity `fn*`, field `set!`, etc. Unknown expr types still hit the **`ExprToBytecode` fallback** (`WARNING: Unimplemented expression…`).

**Suggested next steps:** follow the priority order in **Roadmap: loading `src/clj/clojure/core.clj`** above ( **`loop`/`recur`** first, then **dynamic bindings**, then **monitors**, then **`letfn*`**, then deferred JVM forms, plus **runtime integration**). Within that, the **`Expr`** rows in this table remain the concrete checklist.
...
- `LetFnExpr`: **not** yet mapped. Other mapped forms include **`CaseExpr`**, **`ImportExpr`**, **`QualifiedMethodExpr`**, **`AssignExpr`**, **`KeywordInvokeExpr`** (see **Implemented Expressions**).
