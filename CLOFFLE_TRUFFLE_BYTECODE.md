...
| **2** | **Dynamic `binding` / thread-bound vars** | **`ReadVar`** / **`WriteVar`** use **`Var.get` / `Var.set`** (thread-bound **`set!`**). **`binding`** expands to **`pushThreadBindings`** + **`try`**/**`finally`**; covered by **`StaticMethodExpr`** + **`TryExpr`**. **`BytecodeDslTestSupport.evalBytecode`** wraps each run with **`Clojure.pushEvalThreadBindings`** / **`Var.popThreadBindings`** (same frame as **`CloffleCompiler.compile`**’s outer push). Remaining gaps: **`*out*`** / I/O only if you need them explicitly on the bytecode path beyond **`Compiler`** load. |
...
- **Build**: `clojure -T:build run-bytecode-dsl-tests` runs the three JUnit classes above by default (optional `:fresh`, `:args` for JUnit discovery). **`clojure -T:build run-bytecode-runtime-integration`** runs only **`BytecodeRuntimeIntegrationTest`** (multi-file bootstrap + sequential compile + AOT serialization smoke). Reports under `target/surefire-reports`. Run after bytecode or `Compiler` changes.
...
**Next (integration):** expand **`bootstrap_slice.clj`** / **`bootstrap_extra.clj`** / **`core_mini.clj`** as more **`Expr`** forms land; grow **`require`** / **`load-file`** coverage (real **`ns`** / **`require`** still need **`clojure.core`**); packaged core: **AOT deserialize** in **`build.clj`** beyond the **`run-bytecode-runtime-integration`** serialize round-trip smoke, and **`ClojureLanguage`** startup when you ship a binary core.
