...
2. **`ConstantExpr` (non-empty collections):** `emitConstantValue` wraps any `IPersistentCollection` in `IdentityConstant` — a wrapper whose `equals` uses identity (`this == o`) and `hashCode` uses `System.identityHashCode`. The `LoadIdentityConstant` bytecode operation (with `@ConstantOperand(type = IdentityConstant.class)`) unwraps the value at runtime. This prevents the constant pool from merging collections that are value-equal but type-distinct, while also preventing Truffle's constant-folded `CreateVector`/`CreateSet`/`CreateMap` results from colliding with quoted-collection constants.
