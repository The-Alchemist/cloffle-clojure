...
- `NumberExpr`
- `EmptyExpr`: Empty collection literals (`[]`, `'()`, `#{}`, `{}`) use `emitStaticField` (e.g., `PersistentVector.EMPTY`) instead of `emitLoadConstant` to avoid Truffle constant pool deduplication via `equals()` — see **`EmptyExpr` constant pool deduplication** section.
...
**`EmptyExpr` constant pool deduplication (fixed):** Truffle's Bytecode DSL constant pool deduplicates constants via `Object.equals()`. Clojure's `PersistentVector.EMPTY.equals(PersistentList.EMPTY)` returns `true` (both are empty sequential collections with the same hash). When bytecode loaded both `[]` and `'()` as constants via `emitLoadConstant(ee.coll)`, whichever empty collection was loaded first won — subsequent loads of the other returned the same object. This caused `[]` passed through a closure to arrive as `PersistentList$EmptyList` when `'()` was in scope, breaking `into` (which dispatches on `IEditableCollection`) and producing `PersistentList` results instead of `PersistentVector`. **Fix:** `ExprToBytecode` now mirrors the JVM compiler's `EmptyExpr.emit` by loading empty collections via `emitStaticField` (`PersistentVector.EMPTY`, `PersistentList.EMPTY`, `PersistentArrayMap.EMPTY`, `PersistentHashSet.EMPTY`) instead of `emitLoadConstant`. Each `StaticField` operation has distinct `ConstantOperand` entries (class + field name) that don't collide, and the field value is resolved at runtime.
