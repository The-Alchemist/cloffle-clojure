...
| Pattern | Lowering | Safety |
|---|---|---|
| `(first (map f [x]))` | `(f x)` | Statically non-empty literal |
| `(first (map f coll))` | `(let [s (seq coll)] (when s (f (first s))))` | Dynamic nil-guard |
| `(second (map f [x y]))` | `(f y)` if `f` pure, else `(do (f x) (f y))` | Side-effect order |
