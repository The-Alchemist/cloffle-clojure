...
/**
 * PEA experiment V4 (primitive slots): V3 plus parallel {@code long[]}/{@code double[]}
 * slot arrays so numeric leaves are not boxed into {@code values[]}. No
 * {@code Arrays.fill(values, MISSING)} — {@code states[]} defaults to MISSING.
 */
public final class JsonScanV4PrimSlots {

    private JsonScanV4PrimSlots() {
    }


    /** Result carrier with unboxed numeric slots. Convert via {@link #toBoxed} for decode. */
    public static final class PrimScanResult {
        public static final byte KIND_OBJECT = 0;
        public static final byte KIND_LONG = 1;
        public static final byte KIND_DOUBLE = 2;
        public static final byte KIND_INT = 3;
        ...
        public final byte[] valueKinds;

        public PrimScanResult(byte[] source, int slots) {
            this.source = source;
            this.starts = new int[slots];
            this.lengths = new int[slots];
            this.states = new byte[slots];
            this.values = new Object[slots];
            this.longValues = new long[slots];
            this.doubleValues = new double[slots];
            this.valueKinds = new byte[slots];
            // states defaults to 0 == TypedScanResult.MISSING; no Arrays.fill
        }

        /** Box primitives into a baseline TypedScanResult for decode / parity. */
        public JsonScan.TypedScanResult toBoxed() {
            JsonScan.TypedScanResult out = new JsonScan.TypedScanResult(source, values.length);
            System.arraycopy(starts, 0, out.starts, 0, starts.length);
            System.arraycopy(lengths, 0, out.lengths, 0, lengths.length);
            System.arraycopy(states, 0, out.states, 0, states.length);
            for (int i = 0; i < values.length; i++) {
                if (valueKinds[i] == KIND_INT) {
                    out.values[i] = Integer.valueOf((int) longValues[i]);
                } else if (valueKinds[i] == KIND_LONG) {
                    out.values[i] = Long.valueOf(longValues[i]);
                } else if (valueKinds[i] == KIND_DOUBLE) {
                    out.values[i] = Double.valueOf(doubleValues[i]);
                } else if (states[i] == JsonScan.TypedScanResult.VALUE) {
                    out.values[i] = values[i];
                } else {
                    out.values[i] = JsonScan.MISSING;
                }
            }
            return out;
        }
    ...
    public static PrimScanResult projectString(String json, JsonScan.TypedTrieNode root, JsonScan.TypedLeaf[] leaves) {
    ...
    @TruffleBoundary
    public static PrimScanResult projectBytes(byte[] json, JsonScan.TypedTrieNode root, JsonScan.TypedLeaf[] leaves) {
    ...
    @TruffleBoundary
    public static PrimScanResult projectByteBuffer(ByteBuffer json, JsonScan.TypedTrieNode root, JsonScan.TypedLeaf[] leaves) {
    ...
    public static PrimScanResult projectBytes(byte[] json, int off, int len, JsonScan.TypedTrieNode root,
    ...
    public static PrimScanResult projectBytesPartialEvaluated(byte[] json, JsonScan.TypedTrieNode root,
                                                               JsonScan.TypedLeaf[] leaves, boolean firstWins) {
        if (json == null) {
            throw new NullPointerException("json");
        }
    ...
    /** Bytes-only variant: delegate native TruffleString input to baseline {@link JsonScan}. */
    @TruffleBoundary
...
static final class TypedScanner {
    ...
    PrimScanResult scan(
            ...
            boolean firstWins) {
        this.buf = json;
        return scan(from, to, root, leaves, firstWins, new PrimScanResult(json, leaves.length));
    }

    private PrimScanResult scan(
            int from, int to, JsonScan.TypedTrieNode root, JsonScan.TypedLeaf[] leaves,
            boolean firstWins, PrimScanResult result) {
        this.pos = from;
        this.end = to;
        this.depth = 0;
    ...
    private void projectTypedValue(JsonScan.TypedTrieNode node, PrimScanResult result) {
        if (node.slot >= 0) {
    ...
    private void captureTypedLeaf(int slot, JsonScan.TypedLeaf leaf, PrimScanResult result) {
    ...
    private void captureTypedString(int slot, JsonScan.TypedLeaf leaf, PrimScanResult result) {
     ...
     */
    private void captureTypedAny(int slot, JsonScan.TypedLeaf leaf, PrimScanResult result) {
        if (pos >= end) {
            { CompilerDirectives.transferToInterpreter(); throw err("Unexpected end of JSON", pos); }
        }
        byte c = sourceByte(pos);
    ...
    private void captureTypedAnyNumber(int slot, PrimScanResult result) {
        ...
        if (!isFloat && !overflow) {
            result.longValues[slot] = negative ? -val : val;
            result.valueKinds[slot] = PrimScanResult.KIND_LONG;
            result.states[slot] = JsonScan.TypedScanResult.VALUE;
        } else {
            result.starts[slot] = start;
            result.lengths[slot] = pos - start;
    ...
    private void captureTypedInteger(int slot, JsonScan.TypedLeaf leaf, PrimScanResult result) {
        ...
        if (overflow || (leaf.kind == JsonScan.TypedLeaf.INT && !intRange)) {
            ...
            result.longValues[slot] = finalVal;
            result.valueKinds[slot] = intRange
                    ? PrimScanResult.KIND_INT
                    : PrimScanResult.KIND_LONG;
            result.states[slot] = JsonScan.TypedScanResult.VALUE;
        }
    }
     ...
     */
    private void captureTypedDouble(int slot, PrimScanResult result) {
        if (pos >= end || (sourceByte(pos) != '-' && !isDigit(sourceByte(pos)))) {
        ...
        if (!isFloat && !intOverflow) {
            ...
            result.doubleValues[slot] = d;
            result.valueKinds[slot] = PrimScanResult.KIND_DOUBLE;
            result.states[slot] = JsonScan.TypedScanResult.VALUE;
        } else if (!tooManyDigits) {
            result.doubleValues[slot] = EiselLemire.toDouble(
    ...
    private void projectTypedObject(JsonScan.TypedTrieNode node, PrimScanResult result) {
        pos++;
        enterDepth();
        try {
            skipWs();
    ...
    private void projectTypedArray(JsonScan.TypedTrieNode node, PrimScanResult result) {
        pos++;
        enterDepth();
        int index = 0;
        try {
     ...
     */
    @ExplodeLoop
    private void projectTypedEdge(JsonScan.TypedTrieNode node, int edgeIndex, PrimScanResult result) {
        JsonScan.TypedTrieEdge[] edges = node.edges;
        for (int i = 0; i < edges.length; i++) {
            if (i == edgeIndex) {
    ...
    private static boolean isSubtreeComplete(JsonScan.TypedTrieNode node, PrimScanResult result) {
    ...
    private String parseStringValue() {
        ...
        String s = strEscaped
                ...
                : utf8Slice(buf, start, closeQuote - start);
        pos = closeQuote + 1;
        return s;
    }
    ...
    @TruffleBoundary
    private static String utf8Slice(byte[] buf, int start, int len) {
        if (len == 0) {
            return "";
        }
