...
/**
 * Decimal-to-binary64 conversion for JSON number literals, as the Eisel-Lemire algorithm from
 * Daniel Lemire's "Number Parsing at a Gigabyte per Second" (https://arxiv.org/abs/2101.11408),
 * with the halfway-case refinement of Mushtak and Lemire's "Fast Number Parsing Without Fallback"
 * (https://arxiv.org/abs/2212.06644). Ported from simdjson-java's {@code DoubleParser}.
 *
 * <p>Exact for significands of at most {@link #MAX_SIGNIFICANT_DIGITS} decimal digits, which is
 * the only case the scanner calls it with; anything wider stays on the slower path that re-reads
 * the literal text.
 */
public final class EiselLemire {

    /** A long holds 19 decimal digits without overflow: 9999999999999999999 < 2^64. */
    public static final int MAX_SIGNIFICANT_DIGITS = 19;

    /** Below this power of ten the value rounds to zero in binary64. */
    private static final int MIN_POWER_OF_TEN = -342;
    /** Above this power of ten the value is infinite in binary64. */
    private static final int MAX_POWER_OF_TEN = 308;
    private static final int MIN_POWER_OF_FIVE = -342;
    ...
    private static final int SIGN_BIT_INDEX = 63;
    private static final int SIGNIFICAND_EXPLICIT_BIT_COUNT = 52;
    private static final int SIGNIFICAND_SIZE_IN_BITS = SIGNIFICAND_EXPLICIT_BIT_COUNT + 1;
    private static final int MAX_FINITE_NUMBER_EXPONENT = 1023;
    private static final int MIN_FINITE_NUMBER_EXPONENT = -1022;
    private static final int SUBNORMAL_EXPONENT = -1023;
    private static final long MULTIPLICATION_MASK =
            0xFFFFFFFFFFFFFFFFL >>> SIGNIFICAND_EXPLICIT_BIT_COUNT + 3;

    private static final long[] POWERS_OF_FIVE = {
    ...
    /**
     * Convert {@code significand10 * 10^exp10} to the nearest binary64 value. The significand must
     * carry no more than {@link #MAX_SIGNIFICANT_DIGITS} significant digits.
     */
    public static double toDouble(boolean negative, long significand10, long exp10) {
        if (abs(exp10) < POWERS_OF_TEN.length
                && compareUnsigned(significand10, MAX_LONG_REPRESENTED_AS_DOUBLE_EXACTLY) <= 0) {
            // Small exponents are exact in one hardware multiply or divide. See
            // https://www.exploringbinary.com/fast-path-decimal-to-floating-point-conversion/.
            double result = significand10;
            result = exp10 < 0
                    ? result / POWERS_OF_TEN[(int) -exp10]
                    : result * POWERS_OF_TEN[(int) exp10];
            return negative ? -result : result;
        }
        if (exp10 < MIN_POWER_OF_TEN || significand10 == 0) {
        ...
        // Normalize the decimal significand into [2^63, 2^64), then multiply by 5^exp10 taken from
        // the table; the second limb is only needed when the first product is ambiguous.
        int lz = numberOfLeadingZeros(significand10);
        significand10 <<= lz;
        int index = 2 * (int) (exp10 - MIN_POWER_OF_FIVE);
        long upper = unsignedMultiplyHigh(significand10, POWERS_OF_FIVE[index]);
        long lower = significand10 * POWERS_OF_FIVE[index];
        if ((upper & MULTIPLICATION_MASK) == MULTIPLICATION_MASK) {
            long secondUpper = unsignedMultiplyHigh(significand10, POWERS_OF_FIVE[index + 1]);
            lower += secondUpper;
        ...
        // Extract 54 bits: 53 of significand plus one to round with. The product has 0 or 1
        // leading zeros, so the shift is 9 or 10.
        long upperBit = upper >>> 63;
        long upperShift = upperBit + 9;
        long significand2 = upper >>> upperShift;

        // 10^q contributes floor(q * log2(10)) to the binary exponent, which (217706 * q) >> 16
        // computes over the supported range; the rest undoes the two normalizations above.
        long exp2 = ((217706 * exp10) >> 16) + 63 - lz + upperBit;
        if (exp2 < MIN_FINITE_NUMBER_EXPONENT) {
            if (exp2 <= MIN_FINITE_NUMBER_EXPONENT - 64) {
                return negative ? -0.0 : 0.0;
            }
            significand2 >>= 1 - EXPONENT_BIAS - exp2;
            significand2 += significand2 & 1;
            significand2 >>= 1;
            // Rounding up can lift a subnormal into the smallest normal number.
            exp2 = significand2 < (1L << 52) ? SUBNORMAL_EXPONENT : MIN_FINITE_NUMBER_EXPONENT;
            return bits(negative, significand2, exp2);
        }

        // Exactly halfway between two representable values: round to even.
        if (exp10 >= -4 && exp10 <= 23
                && (significand2 << upperShift) == upper && compareUnsigned(lower, 1) <= 0
                && (significand2 & 3) == 1) {
            significand2 &= ~1;
        }
        significand2 += significand2 & 1;
        significand2 >>= 1;
        if (significand2 == (1L << SIGNIFICAND_SIZE_IN_BITS)) {
            significand2 >>= 1;
            exp2++;
        }
        if (exp2 > MAX_FINITE_NUMBER_EXPONENT) {
            return negative ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY;
        }
        return bits(negative, significand2, exp2);
    }

    private static double bits(boolean negative, long significand2, long exp2) {
        long bits = significand2;
        bits &= ~(1L << SIGNIFICAND_EXPLICIT_BIT_COUNT);
        bits |= (exp2 + EXPONENT_BIAS) << SIGNIFICAND_EXPLICIT_BIT_COUNT;
        return longBitsToDouble(negative ? bits | (1L << SIGN_BIT_INDEX) : bits);
    }
}
