; Slice of bootstrap-style definitions (from core_mini.clj) + smoke tests.
; Uses only host/RT — no clojure.core/+ or clojure.core// so this file loads before core.clj.
...
;; Bootstrap `cond` (core_mini uses clojure.core/cons — use `cons` / `cond` here).
...
;; core.clj-style variadic assoc (let + recur + nnext inside fn*).
(def
 assoc
 (fn ^:static assoc
...
;; Smoke (Numbers/add instead of clojure.core/+ ; ArithmeticException instead of clojure.core// )
(def a 10)
(def b 20)
(if a b :false)

(def add (fn* [x y] (clojure.lang.Numbers/add x y)))
(add 10 20)
...
(try
  (throw (ArithmeticException. "div0"))
...
;; Uses `last` and `hash-map` from this slice; final sum must stay 42 for BytecodeRuntimeIntegrationTest.
(def slice-tail (last (list 1 2 5)))
(clojure.lang.Numbers/add slice-tail 37)
