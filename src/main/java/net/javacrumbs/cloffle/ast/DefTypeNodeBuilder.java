...
import net.javacrumbs.cloffle.nodes.DefTypeNode;
...
public class DefTypeNodeBuilder extends AbstractNodeBuilder {
    private static final Keyword DEFTYPE = keyword("deftype");
    private static final Keyword INTERFACES = keyword("interfaces");
    private static final Keyword FIELDS = keyword("fields");
    ...
    protected DefTypeNodeBuilder(AstBuilder astBuilder) {
        super(DEFTYPE, astBuilder);
    }
    ...
    public ClojureNode buildNode(Map<Keyword, Object> tree) {
        ...
        List<Map<Keyword, Object>> fieldMaps = (List<Map<Keyword, Object>>) tree.get(FIELDS);
        String[] fieldNames = new String[fieldMaps.size()];
        int[] fieldSlots = new int[fieldMaps.size()];
        for (int i = 0; i < fieldMaps.size(); i++) {
            Object name = fieldMaps.get(i).get(NAME);
            fieldNames[i] = name.toString();
            fieldSlots[i] = getSlotIndex(name);
        }
        ...
        return new DefTypeNode(interfaces, fieldNames, fieldSlots, methodDefs,
                getAstBuilder().getLanguage());
    }
