...
public class BindingNodeBuilder extends AbstractNodeBuilder {
    ...
    private static final Keyword VARIADIC = keyword("variadic?");
    ...
    public ClojureNode buildNode(Map<Keyword, Object> tree) {
        ...
        Boolean isVariadic = Boolean.TRUE.equals(tree.get(VARIADIC));
        if (init == null) {
            if (ARG.equals(local)) {
                if (isVariadic) {
        ...
        return BindingNodeGen.create(name, init, getSlotIndex(name));
    }
}
