...
;; (host-interop root-bind). Root restore while those Vars are mocked is covered by
;; test-alter-var-root-under-seq-first-next-nth-redef and
;; test-thread-bindings-under-seq-first-next-nth-redef below — not stock tests.
;; Tier-3 / :cloffle/op lowering intentionally does not route hot call sites through
;; the Var after redef; do not assert the body sees a mock for nth/first/seq/next.
